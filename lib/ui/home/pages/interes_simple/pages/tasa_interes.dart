...
class TasaInteresCalculator {
  ...
  static double calcularTasaInteres(double interes, double capital, double valorTiempo, String unidadTiempo) {
    final tiempoEnAnios = calcularTiempo(valorTiempo, unidadTiempo);
    return interes / (capital * tiempoEnAnios);
  }
}

class TasaInteresPage extends StatefulWidget {
  const TasaInteresPage({super.key});

  @override
  State<TasaInteresPage> createState() => _TasaInteresPageState();
}

class _TasaInteresPageState extends State<TasaInteresPage> {
          ...
          child: Column(
            children: [
              ...
              Padding(
                ...
                child: Text(
                  "Para hallar la Tasa de Interés se utiliza la siguiente formula i = I / (Ct)\nDonde i es tasa de interés, I es intereses, C es capital y t es tiempo",
                  style: GoogleFonts.saira(
                    fontSize: 15,
                    fontWeight: FontWeight.bold
                  ),
              ...
              Container(
                ...
                child: TextButton(
                  ...
                  onPressed: () {
                    if(interesController.text.isEmpty || capitalController.text.isEmpty){
                      showDialog(
                        context: context,
                    ...
                    if(tiempoCompuesto){
                      ...
                      double tasaInteres = TasaInteresCalculator.calcularTasaInteres(interes, capital, valorTiempo, unidadTiempo)*100;

                      showDialog(
                        context: context,
                        builder: (BuildContext context) {
                          return CustomAlertDialog(title: 'Resultado', content: 'Tasa de Interés: ${tasaInteres.toStringAsFixed(2)}%', capitalController: capitalController, tasaInteresController: interesController,);
                        },
                      );
